...
-- | Types and functions for type-level prime compositions, encoded as lists of exponentiated prime factors.
module Data.TypeLevel.PrimeComposition where
...
-- | A type-level representation of a /prime composition/ : a list of exponentiated prime numbers in ascending order [2^/p/, 3^/q/, 5^/r/ ... ].
data exponentiatedPrime ::: tail

-- | A type-level representation of the /empty/ prime composition.
data N
...
-- | Converts composition /x/ into a run-time value.
...
-- | Finds the greatest common divisor of composition /x/ and composition /y/.
...
-- | Multiplies composition /x/ with composition /y/.
class                            Multiply x y z | x y -> z
instance (C x y c, M c x y z) => Multiply x y z

-- | Finds the greatest common divisor of composition /x/ and composition /y/, where
...
-- | Multiplies composition /x/ with composition /y/, where the heads
--   /a/ and /b/ of compositions /x/ and /y/ have relative ordering /c/.
...
-- | Compares the head (/a/ ^ /p/) of composition /x/
--   with the head (/b/ ^ /q/) of composition /y/.
--
--   Returns:
--
--   * 'LT' if (/a/ \< /b/)
